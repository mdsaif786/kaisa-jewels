...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.publishers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.publisher.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Publisher::STATUS_SELECT as $key => $label)
                ...
                @endif
                <span class="help-block">{{ trans('cruds.publisher.fields.status_helper') }}</span>
            </div>
            <div class="form-group">
