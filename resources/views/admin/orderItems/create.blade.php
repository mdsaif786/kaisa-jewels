    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-items.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('cart_item_type') ? 'is-invalid' : '' }}" name="cart_item_type" id="cart_item_type">
                    ...
                    @foreach(App\Models\OrderItem::CART_ITEM_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('cart_item_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gst_amount') ? 'is-invalid' : '' }}" type="number" name="gst_amount" id="gst_amount" value="{{ old('gst_amount', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('combo_offer') ? 'is-invalid' : '' }}" name="combo_offer_id" id="combo_offer_id">
                    @foreach($combo_offers as $id => $combo_offer)
                        <option value="{{ $id }}" {{ old('combo_offer_id') == $id ? 'selected' : '' }}>{{ $combo_offer }}</option>
                    @endforeach
                </select>
