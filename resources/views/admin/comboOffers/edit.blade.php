...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.comboOffer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.combo-offers.update", [$comboOffer->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="offer_title">{{ trans('cruds.comboOffer.fields.offer_title') }}</label>
                <input class="form-control {{ $errors->has('offer_title') ? 'is-invalid' : '' }}" type="text" name="offer_title" id="offer_title" value="{{ old('offer_title', $comboOffer->offer_title) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category_id" id="category_id">
                    @foreach($categories as $id => $category)
                        <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $comboOffer->category->id ?? '') == $id ? 'selected' : '' }}>{{ $category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sub_category') ? 'is-invalid' : '' }}" name="sub_category_id" id="sub_category_id">
                    @foreach($sub_categories as $id => $sub_category)
                        <option value="{{ $id }}" {{ (old('sub_category_id') ? old('sub_category_id') : $comboOffer->sub_category->id ?? '') == $id ? 'selected' : '' }}>{{ $sub_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $comboOffer->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount_amount') ? 'is-invalid' : '' }}" type="number" name="discount_amount" id="discount_amount" value="{{ old('discount_amount', $comboOffer->discount_amount) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount_percentage') ? 'is-invalid' : '' }}" type="number" name="discount_percentage" id="discount_percentage" value="{{ old('discount_percentage', $comboOffer->discount_percentage) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('combo_offer_books') ? 'is-invalid' : '' }}" name="combo_offer_books[]" id="combo_offer_books" multiple>
                    @foreach($combo_offer_books as $id => $combo_offer_book)
                        <option value="{{ $id }}" {{ (in_array($id, old('combo_offer_books', [])) || $comboOffer->combo_offer_books->contains($id)) ? 'selected' : '' }}>{{ $combo_offer_book }}</option>
                    @endforeach
                </select>
