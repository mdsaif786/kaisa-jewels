...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.blockOffer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.block-offers.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('offer_type') ? 'is-invalid' : '' }}" name="offer_type" id="offer_type" required>
                    ...
                    @foreach(App\Models\BlockOffer::OFFER_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('offer_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('general_discount') ? 'is-invalid' : '' }}" type="number" name="general_discount" id="general_discount" value="{{ old('general_discount', '') }}" step="1">
