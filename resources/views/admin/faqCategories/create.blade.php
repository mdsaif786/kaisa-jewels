...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.faq-categories.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.faqCategory.fields.status') }}</label>
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    <option value disabled {{ old('status', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\FaqCategory::STATUS_SELECT as $key => $label)
                ...
                @endif
                <span class="help-block">{{ trans('cruds.faqCategory.fields.status_helper') }}</span>
            </div>
            <div class="form-group">
