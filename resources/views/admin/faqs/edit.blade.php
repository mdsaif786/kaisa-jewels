    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.faqs.update", [$faq->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="faq_category_id">{{ trans('cruds.faq.fields.faq_category') }}</label>
                <select class="form-control select2 {{ $errors->has('faq_category') ? 'is-invalid' : '' }}" name="faq_category_id" id="faq_category_id">
                    @foreach($faq_categories as $id => $faq_category)
                        <option value="{{ $id }}" {{ (old('faq_category_id') ? old('faq_category_id') : $faq->faq_category->id ?? '') == $id ? 'selected' : '' }}>{{ $faq_category }}</option>
                    @endforeach
                </select>
