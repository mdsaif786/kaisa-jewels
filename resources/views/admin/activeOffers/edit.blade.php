...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.activeOffer.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.active-offers.update", [$activeOffer->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="offer_title">{{ trans('cruds.activeOffer.fields.offer_title') }}</label>
                <input class="form-control {{ $errors->has('offer_title') ? 'is-invalid' : '' }}" type="text" name="offer_title" id="offer_title" value="{{ old('offer_title', $activeOffer->offer_title) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('banner_image') ? 'is-invalid' : '' }}" type="text" name="banner_image" id="banner_image" value="{{ old('banner_image', $activeOffer->banner_image) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ActiveOffer::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $activeOffer->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount_bracket') ? 'is-invalid' : '' }}" type="number" name="discount_bracket" id="discount_bracket" value="{{ old('discount_bracket', $activeOffer->discount_bracket) }}" step="1">
