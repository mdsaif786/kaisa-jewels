    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.coupons.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('minimum_amount') ? 'is-invalid' : '' }}" type="number" name="minimum_amount" id="minimum_amount" value="{{ old('minimum_amount', '0') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('coupon_discount_type') ? 'is-invalid' : '' }}" name="coupon_discount_type" id="coupon_discount_type" required>
                    ...
                    @foreach(App\Models\Coupon::COUPON_DISCOUNT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('coupon_discount_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_flash_sale') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_flash_sale" id="is_flash_sale" value="1" {{ old('is_flash_sale', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('coupon_type') ? 'is-invalid' : '' }}" name="coupon_type" id="coupon_type">
                    ...
                    @foreach(App\Models\Coupon::COUPON_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('coupon_type', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('book_category') ? 'is-invalid' : '' }}" name="book_category_id" id="book_category_id">
                    @foreach($book_categories as $id => $book_category)
                        <option value="{{ $id }}" {{ old('book_category_id') == $id ? 'selected' : '' }}>{{ $book_category }}</option>
                    @endforeach
                </select>
