...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.priceBlock.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.price-blocks.update", [$priceBlock->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="block_title">{{ trans('cruds.priceBlock.fields.block_title') }}</label>
                <input class="form-control {{ $errors->has('block_title') ? 'is-invalid' : '' }}" type="text" name="block_title" id="block_title" value="{{ old('block_title', $priceBlock->block_title) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\PriceBlock::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $priceBlock->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('set_order') ? 'is-invalid' : '' }}" type="number" name="set_order" id="set_order" value="{{ old('set_order', $priceBlock->set_order) }}" step="1">
