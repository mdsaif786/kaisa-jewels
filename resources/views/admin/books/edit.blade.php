        ...
        <form method="POST" action="{{ route("admin.books.update", [$book->id]) }}" enctype="multipart/form-data">
            ...
            <div class="row">
                ...
                <select class="form-control select2 {{ $errors->has('sub_category') ? 'is-invalid' : '' }}" name="sub_category_id" id="sub_category_id">
                    @foreach($sub_categories as $id => $sub_category)
                        <option value="{{ $id }}" {{ (old('sub_category_id') ? old('sub_category_id') : $book->sub_category->id ?? '') == $id ? 'selected' : '' }}>{{ $sub_category }}</option>
                    @endforeach
                </select>
          ...
          {{--   row end --}}
            ...
            <div class="form-group">
                @if(!empty($book->image))
                <p>Previous Image</p>
                <p><img src="{{$book->image}}" height="100px" width="100px"></p>
                @endif
            ...
            <div class="row">
                ...
                <input class="form-control {{ $errors->has('mrp') ? 'is-invalid' : '' }}" type="number" name="mrp" id="mrp" value="{{ old('mrp', $book->mrp) }}" step="0.01">
                @if($errors->has('mrp'))
                    <div class="invalid-feedback">
                        {{ $errors->first('mrp') }}
                    </div>
                ...
                <input class="form-control {{ $errors->has('isbn_30') ? 'is-invalid' : '' }}" type="text" name="isbn_30" id="isbn_30" value="{{ old('isbn_30', $book->isbn_30) }}">
                ...
                <input class="form-control {{ $errors->has('isbn_10') ? 'is-invalid' : '' }}" type="text" name="isbn_10" id="isbn_10" value="{{ old('isbn_10', $book->isbn_10) }}">
                ...
                <input class="form-control {{ $errors->has('weight') ? 'is-invalid' : '' }}" type="text" name="weight" id="weight" value="{{ old('weight', $book->weight) }}">
                ...
                <input class="form-control {{ $errors->has('sku') ? 'is-invalid' : '' }}" type="text" name="sku" id="sku" value="{{ old('sku', $book->sku) }}">
                @if($errors->has('sku'))
                    <div class="invalid-feedback">
                        {{ $errors->first('sku') }}
                    </div>
                ...
                <input class="form-control {{ $errors->has('dimensions') ? 'is-invalid' : '' }}" type="text" name="dimensions" id="dimensions" value="{{ old('dimensions', $book->dimensions) }}">
                ...
                <span class="help-block">{{ trans('cruds.book.fields.dimensions_helper') }}</span>
                 ...
                 <div class="col-sm-4">
                     <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('no_of_pages') ? 'is-invalid' : '' }}" type="number" name="no_of_pages" id="no_of_pages" value="{{ old('no_of_pages', $book->no_of_pages) }}" step="1">
                ...
                <select class="form-control {{ $errors->has('book_type') ? 'is-invalid' : '' }}" name="book_type" id="book_type" required>
                    ...
                    @foreach(App\Models\Book::BOOK_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('book_type', $book->book_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.book.fields.book_type_helper') }}</span>
            </div>
                 </div>
                 <div class="col-sm-4">
                     <div class="form-group">
                <label for="book_authors_id">{{ trans('cruds.book.fields.book_authors') }}</label>
                <select class="form-control select2 {{ $errors->has('book_authors') ? 'is-invalid' : '' }}" name="book_authors_id" id="book_authors_id">
                    @foreach($book_authors as $id => $book_authors)
                        <option value="{{ $id }}" {{ (old('book_authors_id') ? old('book_authors_id') : $book->book_authors->id ?? '') == $id ? 'selected' : '' }}>{{ $book_authors }}</option>
                    @endforeach
                </select>
                ...
                <select class="form-control select2 {{ $errors->has('publisher') ? 'is-invalid' : '' }}" name="publisher_id" id="publisher_id">
                    @foreach($publishers as $id => $publisher)
                        <option value="{{ $id }}" {{ (old('publisher_id') ? old('publisher_id') : $book->publisher->id ?? '') == $id ? 'selected' : '' }}>{{ $publisher }}</option>
                    @endforeach
                </select>
                ...
                <select class="form-control select2 {{ $errors->has('stock_status') ? 'is-invalid' : '' }}" name="stock_status_id" id="stock_status_id">
                    @foreach($stock_statuses as $id => $stock_status)
                        <option value="{{ $id }}" {{ (old('stock_status_id') ? old('stock_status_id') : $book->stock_status->id ?? '') == $id ? 'selected' : '' }}>{{ $stock_status }}</option>
                    @endforeach
                </select>
...
{{--   row end --}}
            ...
            <div class="row">
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $book->quantity) }}" step="1">
                ...
                <div class="col-sm-4">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}" type="text" name="condition" id="condition" value="{{ old('condition', $book->condition) }}">
                ...
                <select class="form-control select2 {{ $errors->has('binding_types') ? 'is-invalid' : '' }}" name="binding_types[]" id="binding_types" multiple>
                    @foreach($binding_types as $id => $binding_types)
                        <option value="{{ $id }}" {{ (in_array($id, old('binding_types', [])) || $book->binding_types->contains($id)) ? 'selected' : '' }}>{{ $binding_types }}</option>
                    @endforeach
                </select>
                ...
                <select class="form-control select2 {{ $errors->has('language') ? 'is-invalid' : '' }}" name="language_id" id="language_id">
                    @foreach($languages as $id => $language)
                        <option value="{{ $id }}" {{ (old('language_id') ? old('language_id') : $book->language->id ?? '') == $id ? 'selected' : '' }}>{{ $language }}</option>
                    @endforeach
                </select>
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Book::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $book->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                ...
                <input class="form-control {{ $errors->has('publication_city_or_country') ? 'is-invalid' : '' }}" type="text" name="publication_city_or_country" id="publication_city_or_country" value="{{ old('publication_city_or_country', $book->publication_city_or_country) }}">
                ...
                <input class="form-control date {{ $errors->has('publication_date') ? 'is-invalid' : '' }}" type="text" name="publication_date" id="publication_date" value="{{ old('publication_date', $book->publication_date) }}">
                ...
                <input class="form-control {{ $errors->has('coupon_discount') ? 'is-invalid' : '' }}" type="number" name="coupon_discount" id="coupon_discount" value="{{ old('coupon_discount', $book->coupon_discount) }}" step="0.01">
                ...
                <input class="form-control {{ $errors->has('no_of_qty_for_discount') ? 'is-invalid' : '' }}" type="number" name="no_of_qty_for_discount" id="no_of_qty_for_discount" value="{{ old('no_of_qty_for_discount', $book->no_of_qty_for_discount) }}" step="1">
                ...
                <input class="form-control {{ $errors->has('coupon_code') ? 'is-invalid' : '' }}" type="text" name="coupon_code" id="coupon_code" value="{{ old('coupon_code', $book->coupon_code) }}">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $book->slug) }}">
                @if($errors->has('slug'))
                    <div class="invalid-feedback">
                        {{ $errors->first('slug') }}
                    </div>
                ...
                <input class="form-control {{ $errors->has('meta_keywords') ? 'is-invalid' : '' }}" type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $book->meta_keywords) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}" type="text" name="meta_description" id="meta_description" value="{{ old('meta_description', $book->meta_description) }}">
            ...
            <div class="row">
                ...
                <div class="form-check {{ $errors->has('handpicked') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="handpicked" id="handpicked" value="1" {{ $book->handpicked || old('handpicked', 0) === 1 ? 'checked' : '' }}>
                ...
                <div class="form-check {{ $errors->has('new_arrival') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="new_arrival" id="new_arrival" value="1" {{ $book->new_arrival || old('new_arrival', 0) === 1 ? 'checked' : '' }}>
                ...
                <div class="form-check {{ $errors->has('best_sellers') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="best_sellers" id="best_sellers" value="1" {{ $book->best_sellers || old('best_sellers', 0) === 1 ? 'checked' : '' }}>
                ...
                <div class="form-check {{ $errors->has('all_time_classic') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="all_time_classic" id="all_time_classic" value="1" {{ $book->all_time_classic || old('all_time_classic', 0) === 1 ? 'checked' : '' }}>
