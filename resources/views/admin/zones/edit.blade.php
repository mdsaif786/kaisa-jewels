    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.zones.update", [$zone->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="country_id">{{ trans('cruds.zone.fields.country') }}</label>
                <select class="form-control select2 {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country_id" id="country_id">
                    @foreach($countries as $id => $country)
                        <option value="{{ $id }}" {{ (old('country_id') ? old('country_id') : $zone->country->id ?? '') == $id ? 'selected' : '' }}>{{ $country }}</option>
                    @endforeach
                </select>
                ...
                @endif
                <span class="help-block">{{ trans('cruds.zone.fields.country_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="code">{{ trans('cruds.zone.fields.code') }}</label>
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $zone->code) }}">
                @if($errors->has('code'))
                    <div class="invalid-feedback">
                        {{ $errors->first('code') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.zone.fields.code_helper') }}</span>
            </div>
            <div class="form-group">
