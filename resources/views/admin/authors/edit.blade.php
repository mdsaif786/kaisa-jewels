    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.authors.update", [$author->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('about_author') ? 'is-invalid' : '' }}" name="about_author" id="about_author">{{ old('about_author', $author->about_author) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('short_caption') ? 'is-invalid' : '' }}" type="text" name="short_caption" id="short_caption" value="{{ old('short_caption', $author->short_caption) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\Author::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $author->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
