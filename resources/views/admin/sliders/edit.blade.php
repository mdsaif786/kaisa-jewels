    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sliders.update", [$slider->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slider_name') ? 'is-invalid' : '' }}" type="text" name="slider_name" id="slider_name" value="{{ old('slider_name', $slider->slider_name) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('target_url') ? 'is-invalid' : '' }}" type="text" name="target_url" id="target_url" value="{{ old('target_url', $slider->target_url) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('slider_position') ? 'is-invalid' : '' }}" name="slider_position" id="slider_position">
                    ...
                    @foreach(App\Models\Slider::SLIDER_POSITION_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('slider_position', $slider->slider_position) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
