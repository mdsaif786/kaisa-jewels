...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bookCategory.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.book-categories.update", [$bookCategory->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.bookCategory.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $bookCategory->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent_category') ? 'is-invalid' : '' }}" name="parent_category_id" id="parent_category_id">
                    @foreach($parent_categories as $id => $parent_category)
                        <option value="{{ $id }}" {{ (old('parent_category_id') ? old('parent_category_id') : $bookCategory->parent_category->id ?? '') == $id ? 'selected' : '' }}>{{ $parent_category }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\BookCategory::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $bookCategory->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('set_order') ? 'is-invalid' : '' }}" type="number" name="set_order" id="set_order" value="{{ old('set_order', $bookCategory->set_order) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('show_in_mega_menu') ? 'is-invalid' : '' }}" name="show_in_mega_menu" id="show_in_mega_menu">
                    ...
                    @foreach(App\Models\BookCategory::SHOW_IN_MEGA_MENU_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('show_in_mega_menu', $bookCategory->show_in_mega_menu) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('show_in_menu') ? 'is-invalid' : '' }}" name="show_in_menu" id="show_in_menu">
                    ...
                    @foreach(App\Models\BookCategory::SHOW_IN_MENU_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('show_in_menu', $bookCategory->show_in_menu) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $bookCategory->slug) }}">
                @if($errors->has('slug'))
                    <div class="invalid-feedback">
                        {{ $errors->first('slug') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('meta_keywords') ? 'is-invalid' : '' }}" type="text" name="meta_keywords" id="meta_keywords" value="{{ old('meta_keywords', $bookCategory->meta_keywords) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('meta_description') ? 'is-invalid' : '' }}" type="text" name="meta_description" id="meta_description" value="{{ old('meta_description', $bookCategory->meta_description) }}">
